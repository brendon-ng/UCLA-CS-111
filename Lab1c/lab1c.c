...
int main(int argc, char* argv[]){
  ...
  while((opt=getopt_long(argc, argv, "", args, NULL)) != -1) {
	...
	fprintf(stderr, "Error: Error with getrusage\n%s", strerror(errno));
	exitstatus = max(exitstatus , 1);
      }
    }
	...
	fprintf(stderr, "Error: Error in getrusage\n%s\n",strerror(errno));
	exitstatus = max(exitstatus, 1);
      }
      printf("User time: %f ", (end.ru_utime.tv_sec - start.ru_utime.tv_sec) + (float)(end.ru_utime.tv_usec - start.ru_utime.tv_usec)/1000000);
      printf("System time: %f\n", (end.ru_stime.tv_sec - start.ru_stime.tv_sec) + (float)(end.ru_stime.tv_usec - start.ru_stime.tv_usec)/1000000);
    }
  ...
  if(profile && waited){
    if(getrusage(RUSAGE_CHILDREN, &end) < 0){
      fprintf(stderr, "Error: Error in getrusage\n%s\n",strerror(errno));
      exitstatus = max(exitstatus, 1);
    }
    printf("User time: %f ", (end.ru_utime.tv_sec - start.ru_utime.tv_sec) + (float)(end.ru_utime.tv_usec - start.ru_utime.tv_usec)/1000000);
