...
void shut_down(){
	...
	if(SSL_write(ssl_client, report, strlen(report)) < 0){
		fprintf(stderr, "Error writing shutdown to ssl_client\n");
		exit(2);
	}
...
int main(int argc, char** argv){
	...
	if(ssl_client == NULL){
		fprintf(stderr, "Error: Could not create SSL client structure\n");
		exit(2);
	}

        if(SSL_set_fd(ssl_client, sock) == 0){
		fprintf(stderr, "Error: could not connect SSL to socket\n");
		exit(2);
	}
        if(SSL_connect(ssl_client)<1){
		fprintf(stderr, "Error: could not connect SSL with server\n");
		exit(2);
	}
	...
	if(SSL_write(ssl_client, id_str, strlen(id_str)) < 0){
			fprintf(stderr, "Error: Could not write ID to ssl client\n");
			exit(2);
	}
	...
	while(1){
		...
		if(poll_stat > 0 && (fds[0].revents & POLLIN)){
			char buf[256];
			ssize_t bytes_read = SSL_read(ssl_client, buf, sizeof(buf));
			if(bytes_read < 0){
				fprintf(stderr, "Error reading stdin\n");
				exit(2);
			}
			for(int i = 0; i<bytes_read && com_size < 256; i++){
				if(buf[i] == '\n'){
					command[com_size] = '\0';
					if(log_bool){
		...
		if((cur.tv_sec >= last.tv_sec + period) && started){
			...
			if(log_bool){
				...
				if(SSL_write(ssl_client, report, strlen(report))<0){
					fprintf(stderr, "Error writing temp report to SSL client\n");
					exit(2);
				}
			}
			gettimeofday(&last, NULL);
